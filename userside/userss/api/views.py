...
from userss.producer import publish
...
class PostListingView(APIView):
    def get(self,request):
        publish('product_request',{})
        # response = consume_response()
        # print(response)
        return Response({"response"})

class PostLiking(APIView):
    def get(self, request, *args, **kwargs):
        user_id = self.kwargs.get('u_id')
        post_id = self.kwargs.get('p_id')
        id = {"user":user_id,"post":post_id}
        publish(method="add_like",body=id)
